...
function getBindPosition(root, boneName) {
  let bindPosition = null;
  root.traverse(object => {
    if (bindPosition || !object.isSkinnedMesh || !object.skeleton) return;
    const index = object.skeleton.bones.findIndex(bone => bone.name === boneName);
    if (index < 0) return;
    bindPosition = new Vector3().setFromMatrixPosition(object.skeleton.boneInverses[index].clone().invert());
  });
  return bindPosition;
}
...
AFRAME.registerComponent("avatar-arm-ik", {
  ...
  setup() {
    ...
    for (const side of SIDES) {
      ...
      const bindElbow = getBindPosition(root, side.lower[0]);
      const bindHelpers = side.lower.slice(1).map(name => getBindPosition(root, name));
      const bindWrist = getBindPosition(root, side.hand);
      if (!bindElbow || bindHelpers.some(position => !position) || !bindWrist) return false;
      root.updateWorldMatrix(true, true);
      const bindWristWorld = bindWrist.clone().applyMatrix4(root.matrixWorld);
      ...
      // The male MetaPerson bind rig points its collarbone backward, while the
      // female collarbone points forward. Driving that backward collarbone as
      // part of the arm chain sends the male upper arm through his back. Keep
      // that collarbone anchored and solve from the actual upper-arm head. The
      // female path remains the confirmed shoulder-inclusive chain.
      upperBones[0].parent.getWorldQuaternion(parentQ).invert();
      const shoulderDirectionInParent = upper[0].restDirection.clone().applyQuaternion(parentQ);
      const upperStartIndex = shoulderDirectionInParent.z < 0 ? 1 : 0;
      const chainRootBone = upperBones[upperStartIndex];
      chainRootBone.getWorldPosition(shoulder);
      elbowBone.getWorldPosition(elbow);
      root.getWorldScale(target);
      const rootScale = Math.cbrt(Math.abs(target.x * target.y * target.z));
      const bindLowerLength = bindElbow.distanceTo(bindWrist);
      const lowerLength = bindLowerLength * rootScale;
      const lowerRollWeights = [
        0,
        ...bindHelpers.map(position => Math.min(1, Math.max(0, bindElbow.distanceTo(position) / bindLowerLength)))
      ];
      rigs.push({
        side,
        hand,
  ...
  tock() {
    ...
    for (const rig of this.rigs) {
      rig.chainRootBone.getWorldPosition(shoulder);
      ...
      rig.upper[0].bone.parent.getWorldQuaternion(q);
      ...
      for (let i = rig.upperStartIndex; i < rig.upper.length; i++) aim(rig.upper[i], upperDir);
      rig.chainRootBone.updateWorldMatrix(true, true);
